...
const commands = {
	...
	{
		cmd: 'help',
		res: 'Available commands:<br><br><span class="red"># <span class="red">clear</span> - Really, it just clear. <br># <span class="red">screenfetch</span> - Fetches system information in the terminal.<br># <span class="red">links</span> - Links for others sites.<br># <span class="red">music [play/stop/link]</span> - Yung Spinach Cumshot - Subway Sexists <br># <span class="red">visitors</span> - How many times people visited this site.<br></span>',
	},
	sex: 
	{
		cmd: 'is the site skidded?!?11',
		res: 'yes i stol it from qoft :trol:'
	}
}

var prephraseRoot = '<span class="red">root@paddy.wtf</span>:<span class="blue">~</span># ';
var prephrase = '<span class="red">guest@paddy.wtf</span>:<span class="blue">~</span># ';
...
{
	var res = '';
	if (command === './welcome.sh') res = 'wlc to <span class="red">paddy.wtf</span>!<br></span><br>Made By:</span><span class="red"> gob</span><br>Total Visits: <span class="red">' + visits + '</span>'
	else
	{
			...
			if (vInput.startsWith("./")) {
				if (vInput === "./root/welcome.sh") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>paddy.wtf: ' + vInput + ': No such file or directory</p></div>'
					});
				}
				...
				if (typeof loginArray[2] !== "undefined") {
					if (loginArray[2] === "visitor") {
						...
						res = 'paddy.wtf: No passwd entry for user ' + loginArray[2] + '';
					}
					launchCommandAsVisitor({
				...
				if (vInput === "music play") {
					...
					var win = window.open("https://www.youtube.com/watch?v=uFPu4Gfau2o", "_blank");
					win.focus();
					launchCommandAsVisitor({
						cmd: vInput,
						res: ''
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>[ <span class="red">paddy.wtf</span> ] Usage: music [play/stop/link]</p></div>'
					});
				}
...
hiiiiiiiiiiiiiii
				...
				launchCommandAsVisitor({
					...
					launchCommandAsVisitor({
							cmd: $('input').val(),
							res: '<div><p><span class="red">guest@paddy.wtf</span><br><span class="red">OS:</span> Arch Linux ' + OSVer + '<br><span class="red">Kernel:</span> ' + Kernel + '<br><span class="red">Uptime:</span> ' + upTime + '<br><span class="red">Shell:</span> ' + Shell + '<br><span class="red">Disk:</span> ' + Disk + '<br><br><br><br><br><br></p></div>'
						});
			} else if (vInput.trim() !== "") {
		...
		if (root > 0) {
			launchCommandAsRoot({
				cmd: $('input').val(),
				res: '<div><p>paddy.wtf: ' + $('input').val() + ': command not found</p></div>'
			});
		} else {
			launchCommandAsVisitor({
				cmd: $('input').val(),
